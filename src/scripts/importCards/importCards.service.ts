...
export class ImportCardsService {
    constructor(@InjectModel(Card.name) private cardModel: MongooseModel<Card>) { }
    ...
    async insertFromFile(filePath: string): Promise<void> {
        ...
        try {
            ...
            data.forEach(cardData => {
                if (cardMap.has(cardData.card_id)) {
                    ...
                    cardMap.set(cardData.card_id, {
                        ...
                        power: cardData.power === "-" ? 0 : parseInt(cardData.power),
                        counter: cardData.counter === "-" ? 0 : parseInt(cardData.counter),
            ...
            const newCards = Array.from(cardMap.values()).filter(card => !existingCardIds.has(card.card_id));

            if (newCards.length > 0) {
                // Inserta todas las nuevas cartas en la base de datos
                await this.cardModel.insertMany(newCards);
                console.log(`${newCards.length} unique cards inserted successfully.`);
            } else {
